...
tasklist = pd.read_csv('projA_input1.csv',sep=',',index_col='Veh_No')
...
def new_tasks(single_pending,single_processinglist,TimeRem,TaskRem,ResourcePool,t,car):
...
eventlist.starttime = [t+dt.timedelta(days=np.float64(k-1)) for k in tasklist.FirstDay.values]
eventlist.endtime = [t+dt.timedelta(days=np.float64(k-1)) for k in tasklist.FirstDay.values]
...
for car in checklist.index:
    carsob = eventlist.iloc[car-1]['endtime']
    checklist = checklist.set_value(car,'BIWSOB',carsob>=t)
...
while stopflag>0:
   ...
   # for each car, search for pending tasks, update new events
    for car in checklist.index:
        ...
        car_new = new_tasks(car_pending,car_processinglist,TimeRem,TaskRem,ResourcePool,t,car)
        ...
        if not(car_new is 'None'):
            for ind in car_new.index:
                ...
                gantt_s = gantt_s.set_value(car,car_newtemp['task'],t)
                gantt_e = gantt_e.set_value(car,car_newtemp['task'],t+dt.timedelta(minutes=duration_temp))
                gantt_d = gantt_d.set_value(car,car_newtemp['task'],duration_temp)                
